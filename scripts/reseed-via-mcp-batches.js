...
const profileChunks = chunkArray(profiles, 300);
...
profileChunks.forEach((chunk, idx) => {
  ...
  parts.push(`INSERT INTO public.profiles (id, email, full_name, department, year, phone, role, roll_number)
VALUES
${vals}
...
parts.push(`INSERT INTO public.teams (id, name, category, leader_id, status)
VALUES
${teamVals}
ON CONFLICT (id) DO UPDATE SET
  name = EXCLUDED.name,
...
const memVals = memberships.map((m, idx) =>
  `('mem_${idx + 1}', ${esc(m.teamId)}, ${esc(m.userId)}, ${m.isLeader ? "true" : "false"})`
).join(",\n");

parts.push(`INSERT INTO public.team_members (id, team_id, user_id, is_leader)
VALUES
${memVals}
...
parts.forEach((pSql, idx) => {
  fs.writeFileSync(`mcp_part_${idx + 1}.sql`, pSql);
});

console.log(`Generated ${parts.length} part files (mcp_part_1.sql to mcp_part_${parts.length}.sql)!`);
